...
function [freqs, time_axis, freq_axis] = MyStft(in, samp_freq, wlen, fft_len, plotme) 
    ...
    time_axis = (0:1:num_wd-1) * wlen/samp_freq;
    %time_axis = (0:1:num_wd-num_wd/samp_freq) * 1/samp_freq;                             % generating the time axis of the stft
    ...
    elseif plotme == false
        ...
        err("Unrecognized verbose plotme");
    end
...
[spectrum, time, freq] = MyStft( y( 1 : 1e4, :), fs, Sz, fft_len, true);
% Extract the STFT results for each microphone channel

%[~, ~, freq_axis] = MyStft(y, fs, Sz, fft_len);
...
for i = 1:num_steps
    ...
    %{
    imagesc(time_axis, freq_axis, abs(spectrum(:, :, 1)));
    axis xy;
    ylim([0 500]);
    title(sprintf("Channel 1"));
    xlabel("Time (s)");
    ylabel("Frequency (Hz)");
    %}
